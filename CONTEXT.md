...
### Rust (7th language — 20-version sparse language axis × 5 frameworks/9 real major buckets × 8 crypto libs/15 version buckets)

Chosen over Ruby specifically because it doesn't share Go's disqualifying flaw: Rust's standard library has **no built-in crypto at all** — every crypto library is a separately-versioned Cargo crate, architecturally identical to Python/Node/Java/.NET/PHP (confirmed via real crates.io/Docker Hub research, not assumed), unlike Go where the crypto lib was tightly coupled to the compiler version.

**Language-version axis is deliberately sparse, not dense like every other language here** — the one intentional exception to this project's own "density from the start" policy. Every rustc minor from 1.31 (2018-12-06, the oldest pullable `rust:X.Y-slim` tag) through 1.97 is recorded (68 entries), but only a ~20-version subset is `include:true`: the union of every framework/library MSRV floor actually in this registry, plus Debian codename transitions (1.55 bullseye, 1.72 bookworm — see below) and the edition-2024 boundary (1.85). This is provably complete, not just "probably fine": for any (framework, library) pair the language version that makes it work is `max(fw_floor, lib_floor)`, and the max of two numbers already in the sparse set is always itself in the sparse set — verified programmatically (0 uncovered framework×library pairs) after every registry change. Deliberate departure from every other language's dense per-minor axis: language versions aren't dashboard-editable by design (unlike framework/library include/exclude, which goes through `version_overrides`), so expanding the sparse set later is an accepted manual step, not a gap to engineer around.

| Framework | Real major buckets | Notes |
|-----------|--------------------|-------|
| Iron | 0.6 | Single legacy pre-async bucket, abandoned since 2019. Its own real floor is rustc 1.55 (corrected twice, 1.31→1.40→1.55 — its `url`/`idna`/`unicode-normalization` chain needs `tinyvec` 1.x, which itself needs `core::mem::take` (1.40) and later `Array::map`+const generics (1.55), none of it declared in tinyvec's own metadata at all). |
| Rocket | 0.4, 0.5 | 0.4 is nightly-only (needs unstable `proc_macro_hygiene`/`decl_macro`, never stabilized in the form Rocket used) — pinned to a fixed nightly toolchain via rustup regardless of the selected stable `rust_ver` axis value, making that axis nominal/cosmetic for this one combo only. 0.5 is the modern stable/async era. |
| actix-web | 3, 4 | tokio 0.2→1.0 rewrite between majors. Real floor for BOTH majors is rustc 1.75 (vendor-declared 1.42/1.54 both proved too optimistic against real builds). First framework to pull in a modern DNS/URL-parsing stack (`actix-connect`→`trust-dns-proto`→`idna`→icu4x) — see the ecosystem-hub note below. |
| axum | 0.6, 0.7 | Weaker API-shape difference than Rocket/actix-web's breaks. 0.6's floor corrected 1.63→1.69 (a real, accepted transitive floor from the same icu4x family, not routed around further). 0.7's floor corrected 1.70→1.75 (same yoke/icu4x wall as warp, below). |
| warp | 0.1, 0.2, 0.3, 0.4 | Never declared an MSRV, ever, across any major. 4 real API eras, its first-ever floor determined empirically (1.54). 0.4's 2025 rewrite gates `warp::serve`/hyper behind a non-default `server` Cargo feature. |

| Crypto library | Version buckets | Notes |
|-----------------|------------------|-------|
| ring | 0.16, 0.17 | General-purpose, hybrid C+Rust. The "unmaintained" RUSTSEC-2025-0007 advisory was withdrawn — treated as actively maintained. |
| rsa | 0.8, 0.9 | RustCrypto. Real vulnerable/fixed pair for the Marvin Attack timing side-channel (RUSTSEC-2023-0071): "0.8" vulnerable, "0.9" resolves to the patched 0.9.7+. |
| ed25519-dalek | 1, 2 | Real 1.x→2.x API break (`Keypair` → `SigningKey`). `SigningKey::generate()` needs an explicit `features=["rand_core"]` not in the crate's defaults — a genuine our-own-Cargo.toml bug, not an MSRV issue (reproduced identically across 3 different rustc versions before concluding that). |
| sodiumoxide | 0.2 | libsodium bindings, confirmed archived by its own GitHub repo (2022-09-04). Needs `pkg-config` in sys_deps AND `SODIUM_USE_PKG_CONFIG=1` — installing the system package alone silently falls back to an unsupported from-source vendored build. |
| bcrypt | 0.5, 0.10, 0.15 | "0.5" is genuinely unbuildable (its `blowfish ^0.4` dependency needs `block-cipher-trait ^0.6`, and all three published 0.6.x releases are yanked from crates.io — registry-excluded, not pinned around, matching this project's genuine-impossibility convention). |
| argon2 | 0.1, 0.3, 0.5 | RustCrypto. "0.1"'s real API is a 5-arg legacy `PasswordHasher` signature, not the 2-arg one every later bucket uses — needed its own era-aware touch-code dispatch. Real floor is rustc 1.42 (its `base64ct` dependency has used a subslice pattern requiring 1.42 since base64ct's first-ever release — a permanent ecosystem floor, not a bug). |
| rust-crypto (DaGenix) | 0.2 | Genuinely abandoned since 2016, own README security warning — the strongest "legacy/insecure but real" entry, parallels old pycryptodome/phpseclib elsewhere. |
| openssl | 0.10 | System OpenSSL bindings, one long 0.10.x line since ~2016, not a multi-era crate — parallels Python's pyOpenSSL. |

**The core, project-defining bug: no committed Cargo.lock means every build re-resolves transitive dependencies fresh, and crates.io keeps moving the ecosystem forward underneath old targets.** A widely-used transitive crate eventually bumps to a newer Rust edition (edition2024 stabilized at rustc 1.85) than an old, previously-passing `rust_ver` can parse at all — a hard manifest error, not a soft "try older," and it gets MORE common over time purely from upstream churn, unrelated to anything in this project's own registry. Fixed with `msrv_repair.py` (embedded in `lang_rust.py`, run in a throwaway `lockgen` Docker stage on `rust:1-slim` — always the newest floating tag, not the target version): resolve normally with modern cargo, walk the *feature-aware* reachable dependency closure (`cargo metadata --filter-platform`, BFS from the root — a naive "every raw node" walk force-activates dependencies gated behind unrequested optional features), downgrade any crate whose real floor (declared `rust_version`, `edition`, or — critically — cargo's own index schema version for namespaced/weak-dependency features, invisible to every other check) exceeds the target, then exact-pin the entire closure into Cargo.toml (aliased via Cargo's `package = "..."` rename syntax when 2+ incompatible majors of one crate resolve at once). See the general findings below (in "Conventions to follow") for the reusable lessons this surfaced — they generalize well beyond Rust.

**A genuine, recurring ecosystem pattern: MSRV "hub" crates (serde/serde_derive/syn/quote/proc-macro2/tinyvec/subtle, then separately an ICU4X/Unicode-normalization family reachable via any modern `url`/DNS-resolution stack) keep bumping their edition or real (often UNdeclared) compiler-feature floor, well past what their own advertised `rust_version` claims.** Each was found only by a real, failing `docker build` against a previously-passing combo, never anticipated from metadata alone. The wall is reached differently per framework (actix-web/axum via `idna`/`trust-dns-proto`; warp reaches the SAME wall via its own dependency shape) and can be **library-dependent even within one framework bucket** — e.g. warp 0.2 hits the wall at rustc 1.69 generally, but at rustc 1.61 specifically when paired with `ed25519-dalek` (bisected directly: 1.60 clean, 1.61 broken). Established fix pattern: bisect the real boundary empirically, then either cap the specific hub crate (if a compatible older release exists) or remove the affected rustc range from that framework/library bucket's own `compatibility` array (if it's a genuine, permanent floor with nothing to route around) — never assume one threshold covers an entire framework's whole bucket.

**Build-verification status.** Every framework/library combination that generates at least one image has had at least one real combo built, run, and curled on both endpoints — Iron (after 2 floor corrections), Rocket 0.4/0.5 (after a 9-fix chain including a mis-dated pinned-nightly toolchain and a nightly-only SIMD auto-detection issue), actix-web 3/4 (after 4 `msrv_repair.py` bugs and a floor correction), axum 0.6/0.7 (after a duplicate-alias bug and its own icu4x floor correction), and all 4 warp buckets (after the rate-limit-blindness fix, a persistent cross-build fetch cache, and 2 rounds of the icu4x/yoke wall). The full phase-by-phase history (26+ phases) is tracked in Claude's own session memory under "Rust implementation" if a specific combo's fix ever needs re-deriving.

### Ruby (8th language — 15 dense lang versions × 6 frameworks/24 real major buckets × 9 crypto libs/26 version buckets ≈ 3,746 images)

Built in one pass (registry + generator + wiring together, matching the "density from the start" / "check_updates.py is mandatory" conventions this document itself now states) rather than a milestone-first-then-expand approach. Every version/date below was fetched live from Docker Hub's tag-list API and RubyGems.org's `/api/v1/versions/{gem}.json`, not assumed from general knowledge — general Ruby-ecosystem knowledge turned out to be stale or wrong in several concrete places documented below.

**Language versions: dense, 1.9 through 4.0** (`ruby:{ver}-slim`), covering **6 real Debian-codename transitions** — more than any other language in this project has hit (wheezy → jessie → stretch → buster → bullseye → bookworm → trixie). wheezy/jessie/stretch/buster are all EOL and need the same `archive.debian.org` apt-source-rewrite fix already used for old Go/Node/PHP bases. **A genuine one-off finding**: "Ruby 3.5" was never a real release — it was the development codename that got renamed to **4.0.0** at GA (2025-12-25, Ruby's 30th-anniversary Christmas release) — Docker only ever published `3.5.0-preview1`/`3.5-rc` tags under the old working name, recorded here as an explicit `include:false` reference row rather than a real version.

| Framework | Real major buckets | Notes |
|-----------|--------------------|-------|
| Rails | 0 (2004), 1 (2005), 2 (2007) reference-only; 3, 4, 5, 6, 7, 8 real | Majors 0-2 predate Rails' own adoption of Bundler (introduced at 3.0, 2010) — same "no dependency-manager era" reasoning already used for PHP's pre-Composer Laravel 1-3. **A real RubyGems.org data-quality bug found and corrected**: every 0.x/1.x version (and most of 2.x) carries an identical, fabricated `created_at` of 2009-07-25 — RubyGems.org's own 2009 launch date, bulk-backfilled onto pre-existing gems, not each version's real release date. Corrected via real historical dates (0.5=2004-07-24, 1.0=2005-12-14, 2.0=2007-12-07). Rails 5+ ships a real, lighter `--api`-only boot mode (reduced middleware/routing stack, no views/assets/sessions) — a genuine touch-code-relevant difference, but a boot flag on the same gem version, not a separate registry bucket. |
| Sinatra | 0, 1, 2, 3, 4 | Classic top-level `get "/" do...end` DSL confirmed stable and unchanged all the way back to its earliest real tag (0.9.6) — one template covers every major. |
| Grape | 0, 1, 2, 3 | API-only framework, mounted as a Rack app; 3.0 is only a few weeks old as of this writing. |
| Hanami | 0, 1, 2, 3 | 1→2 is a confirmed real full-stack rewrite (moved off the classic Rack-app-object model) — needed its own boot-code era, unlike Sinatra/Grape/Roda/Padrino which each needed only one template. Bucket "0" turned out to share Hanami 1's own API exactly (confirmed via source), not a separate reference-only row the way Rails 0-2 are. |
| Roda | 3 (only) | Stayed on major 3 since 2017 for 9+ years via serial minor bumps (currently 3.106) instead of semver majors — tracked as one long-lived bucket, same reasoning as Rust's Iron. |
| Padrino | 0.x (only) | Built on Sinatra; never left major 0 in 17 years. |

| Crypto library | Version buckets | Notes |
|-----------------|------------------|-------|
| openssl | 2, 3.0, 3.1, 3.2, 3.3, 4 | A RubyGems "default gem" — bundled with the interpreter but genuinely independently versioned/upgradable, the same qualification pattern confirmed for every other language (and specifically the reason Ruby does NOT share Go's disqualifying crypto-coupled-to-toolchain problem). Needed 6 buckets, not 3 — its own Ruby-version floor genuinely rises mid-major (3.0/3.1 need ≥2.6.0, 3.2/3.3 need ≥2.7.0), confirmed per-minor via each version's own latest-patch gemspec, not assumed uniform across a major. |
| rbnacl | 1 through 7 | Pure-FFI libsodium binding (no C-extension compile of its own) — needs `libsodium-dev`/`libsodium23` as a real system dependency. Whether wheezy's own archive actually carries a `libsodium23` package at all (libsodium predates Debian packaging it that far back) is flagged as unverified without a real build attempt. |
| bcrypt | 3 (only) | Flat single-major line since 2014, no real major-version history to speak of. |
| jwt | 0, 1, 2, 3 | **A genuinely unusual, currently-live finding**: 2.x and 3.x are BOTH actively published in parallel right now (maintainers are backporting to the 2.x branch concurrently with 3.x development) — a real dual-maintained-branch situation, not a data artifact. |
| argon2 | 0, 1, 2 | |
| digest | 1, 3 (no 2.x ever published) | Confirmed as a real, independently-gemified default gem (not merely built-in) — the 1→3 jump is a deliberate renumbering to match Ruby's own internal versioning, re-verified against the full raw version list, not a data-fetch gap. |
| roqs | 0.x | The Ruby equivalent of liboqs-python/liboqs-go/liboqs-node/php-liboqs/LibOQS.NET — an actively-maintained direct FFI binding to a liboqs shared library. **Needs a non-standard library-loading mechanism**: locates liboqs via `ENV['ROQS_LIBOQS_DIR']/{uname -m}/liboqs*`, NOT the standard ldconfig/`/usr/local/lib` search every other liboqs consumer in this project relies on — handled by copying the built `.so` into `/opt/roqs-lib/$(uname -m)` and setting that env var. Confirmed working end-to-end via a real build+run: a Kyber768 (ML-KEM-equivalent) keypair generates with no error. |
| jwt-pq | 0.x | Adds ML-DSA-44/65/87 post-quantum signatures to the ruby-jwt ecosystem, with an optional hybrid EdDSA+ML-DSA mode. **Vendors and compiles its own copy of liboqs 0.16.0 from source automatically at `gem install` time** (downloads+checksums a tarball itself, no git clone needed on our side) — redirected via `JWT_PQ_USE_SYSTEM_LIBRARIES=1`/`OQS_LIB=/usr/local/lib/liboqs.so` to reuse this project's own already-built liboqs instead of compiling a second redundant copy. |
| pqc_rails | 0.x | Native FFI to liboqs exposing NIST ML-KEM/ML-DSA, explicitly for existing Rails apps. **Hard-requires a real Rails ≥7.1 host application** (`actionpack`/`activerecord`/`railties` runtime deps, `>=7.1,<9`) — a genuine cross-dependency between the framework and library axes, which this project's generator otherwise treats as fully independent. Enforced explicitly in `write_context()` (skip + rmtree stale, mirroring PHP's Laravel-4×phpseclib precedent) — only ever paired with Rails majors 7/8, every other framework combination is correctly skipped with a clear reason. |

**Other real findings baked into the generator, not just the registry:** Rack 3.0 extracted `rackup`/`Rack::Server`/`Rack::Handler` into a separate gem (confirmed via Rack's own CHANGELOG) — needed as an explicit dependency for Sinatra 4/Padrino/Rails 7-8/Grape 2-3/Hanami 3, verified per-framework against real gemspecs rather than assumed uniform. Bundler is not preinstalled on the old `ruby:1.9/2.0/2.1-slim` bases, and its own Ruby-version floor keeps rising — resolved by picking the newest Bundler that RubyGems' own per-version `ruby_version` field confirms still supports the target Ruby, rather than a hardcoded cutoff table. liboqs 0.16.0 (2026-07-09) is the real current stable tag, not the 0.15.0 pin still used elsewhere in this project for older bindings — re-verified fresh rather than copied forward.

**Build-verification status.** Real `docker build`+`run`+curl (both endpoints) confirmed for: Sinatra 4 + jwt (baseline, no native extensions), Rails 8 + argon2 (native-extension gem + a hand-written minimal `Rails::Application`/`config.ru` boot, deliberately lighter than scaffolding a full app via `rails new`), and Sinatra 4 + roqs (the full liboqs-from-source build stage plus roqs' own non-standard library-path handling). A real local generation pass (3,746 image contexts, 0 exceptions) was also run and spot-inspected across several other framework/library/Debian-era combinations before any Docker build was attempted. Every other combination shares this same template/Dockerfile-shape machinery per framework/library, so an untested combination's risk is concentrated in whichever specific branch it exercises (a particular old Debian codename, a particular native extension, Hanami's 3 distinct eras), not the shared plumbing.

**Post-implementation hardening (2026-08-03 to 2026-08-05): a real batch-test pass found ~30 additional bugs the initial build-only verification above didn't catch — the general lessons, reusable for any future language:**
- **A gem's own declared `ruby_version`/dependency-version floor is frequently just wrong, not merely absent** — the same "maintainer courtesy, not enforced" lesson Rust's MSRV work already documented, confirmed independently and repeatedly for Ruby: `securerandom`/`cgi`/`logger`/`timeout` all declared an unrestricted or too-low floor while their own code called a method Ruby itself only added several versions later (`Random.urandom` at 2.5, `String#delete_prefix` at 2.5, `&.` at 2.3) — only caught by actually booting the app and hitting a real request, never by the resolver. **A passing `bundle install` (or `docker build`) proves the dependency graph resolves, nothing about whether the resolved code actually runs** — the same lesson .NET's own CONTEXT.md section states about a passing build proving nothing about runtime behavior.
- **A version-comparison bug can silently corrupt every era-based resolution decision at once.** This project's own `_lang_ver_tuple`/`_ruby_floor_tuple` helpers produced tuples of different lengths ("2.5" → `(2, 5)` vs ">= 2.5.0" → `(2, 5, 0)`), and plain Python tuple comparison treats a shorter tuple as always less than a longer one even when every shared component is equal — silently picking an unnecessarily older release than needed at exact version-boundary cases. Fixed by padding asymmetrically: the actual-runtime-version side with a high sentinel (the registry only ever records "major.minor", but the real container always runs whichever is the latest actual patch of that line), the requirement/floor side with zero. Worth an explicit unit-style sanity check for any future language's own equivalent version-tuple helper.
- **An "unconstrained transitive dependency floats to whatever's newest and eventually outpaces an older target" bug reliably recurs framework-wide, not as isolated one-offs** — fixing Rails' own bundler-ceiling issue surfaced a chain of 20+ further such dependencies one at a time (each only visible once the previous was fixed): `rake`, `concurrent-ruby`, `multi_json`, `rack-cache`, `thor`, `webrick`, `rexml`, `minitest`, `nokogiri`, `racc`, `logger`, `mail`, `i18n`, `rails-dom-testing`, `arel`, `globalid`, `method_source`, `rack-test`, `rack`, `nio4r`, `websocket-driver`/`-extensions`, `marcel`, `sprockets`, `bigdecimal`, `benchmark`, `base64`. A shared `_era_gem_version(gem, lang_ver, fallback, below=None)` helper (live RubyGems floor lookup, with an optional upper-bound cap for the handful of cases where a framework's OWN gemspec caps a dependency below some major line) absorbed nearly all of them — worth building this kind of shared helper on the FIRST pass for any future language with the same "package manager resolves transitively, no per-image lockfile" shape (already flagged as a general risk in the Rust lessons above), rather than writing one-off pins reactively.
- **The same dependency name can need a genuinely different version cap depending on WHICH major of the calling framework is asking**, not just which target runtime version — `mail`, `rack`, `globalid`, and `marcel` each had one framework major (Rails 3, or Rails 5) with a measurably tighter constraint (`~> X.Y.Z` vs `~> X.Y`, or an extra transitive requirement like `activesupport >= 6.1`) than every other major shared the same dependency with. Confirmed via each major's own real gemspec metadata, not assumed uniform — check this explicitly whenever a shared dependency-pinning helper is reused across multiple framework majors.
- **A stdlib library getting demoted from a Ruby default gem to a separately-installed "bundled gem" is a recurring event across MULTIPLE Ruby minor releases, not a one-time special case** — `ostruct`/`fiddle` at 4.0, `webrick`/`rexml` at 3.0, `bigdecimal` at 3.4, `benchmark`/`logger`/`base64` at 4.0. Each needs an explicit `gem "x"` line added ONLY once the target Ruby version actually clears that specific removal point (adding it unconditionally can reintroduce an unrelated bug by loading a standalone release that behaves differently from the specific Ruby version's own bundled copy — confirmed for `bigdecimal` specifically, see the Rails major 3/4 ceiling note above). **This fix needs to be framework-agnostic, not just wired into whichever framework happened to surface it first** — `logger`/`base64` were originally only fixed inside Rails' own code path and silently left every other framework (Roda, Sinatra, Grape, Padrino, Hanami) still broken at the same Ruby versions until a later, separate batch run caught it.
- **A hardcoded gem-install filesystem path is fragile to which Bundler/RubyGems version is in play, not just which Ruby version.** `argon2`'s own native-extension relink step assumed a flat `/usr/local/bundle/gems/<name>-<ver>/` path, but some Bundler/RubyGems combinations (confirmed for Ruby 2.3 + Bundler 2.3.27) nest gems one level deeper, under `/usr/local/bundle/ruby/<api_version>/gems/...` — fixed with a `find`-based lookup instead of a hardcoded path. The identical root cause also broke Sinatra's own `CMD ["ruby", "app.rb"]` (no `bundle exec`), which relies on RubyGems' own implicit gem activation rather than Bundler's explicit one — fixed by running every framework via `bundle exec`, not just the ones already doing so.
- **Framework majors frequently have a real, narrower Ruby ceiling AND floor than their own gemspec-declared range implies, only visible from a real request, not a build** — Rails 3/4's ceiling (Ruby 2.7 removes `BigDecimal.new`, still called unconditionally by their resolved activesupport patch) and Rails 5's both ends (floor: Ruby 2.2-2.4 opened an endless tail of the unconstrained-dependency pattern above with no economical fix; ceiling: Ruby 3.0's keyword-argument separation genuinely breaks Rails 5.2's own middleware-stack code, since Rails 5.2 reached EOL before Ruby 3.0 shipped and was never patched for it) were each independently narrowed after a full test-harness pass, not just a build pass, exposed real boundaries the original build-only verification had missed entirely.

### Kotlin (9th language — 15 dense lang versions × 5 frameworks/19 real major buckets × 6 crypto libs/57 version buckets ≈ 3,325 images, after post-v1.10 hardening — see below)

Built in two passes (Ktor/Spring Boot first, then http4k/Micronaut/Quarkus added on request after the user asked "only 2 frameworks?" — a deliberate under-scoping on the first pass that's worth naming: don't assume a 2-framework rollout is "done" for a language without checking whether the user wants broader coverage). Registry + generator + wiring built together each pass, same "density from the start" / "`check_updates.py` is mandatory in the same pass" approach as Ruby's. **Language-version axis is the Kotlin COMPILER version (1.0 through 2.4, 15 real minor lines), not JDK** — a deliberate, user-confirmed design decision: there is no official Kotlin Docker image (confirmed via a direct registry-API 404), and `kotlinc` itself has needed only JDK 8 to run since 1.1.2 (2017) with no rise since, even through the K2 rewrite. JDK/Gradle-builder-image selection is instead a pure implementation detail (`lang_kotlin.py`'s `_TOOLCHAIN` dict, one `(jdk, gradle-image-tag)` pair per Kotlin line, every exact tag confirmed live against Docker Hub before being hardcoded) — never a registry field, mirroring how this project already treats other languages' OS-base-image selection as generator-internal logic. Reuses `lang_java.py`'s Maven Central resolution helpers directly (`_fetch_maven_versions`/`_resolve`/`_release_date`/`MavenLookupError`) rather than reimplementing them, since every Kotlin framework/library tracked here lives on Maven Central too — `check_updates.py`'s own "maven" `fetch_kind` already hardcodes an import of `lang_java` for exactly this reason, so this keeps both paths byte-for-byte consistent. **`db.py` has its OWN separate `_REGISTRY_FILES` list, distinct from `check_updates.py`'s** — missed on the very first wiring pass, caught only because a post-deploy `/api/init` resync reported 0 Kotlin images despite 1,161 real contexts already on disk; any future language must wire both lists, not just one.

| Framework | Real major buckets | Notes |
|-----------|--------------------|-------|
| Ktor | 1, 2, 3 | The flagship Kotlin-native framework. **A genuine, build-breaking finding**: each Ktor MINOR pins its own required Kotlin compiler version (2.0.x→1.6.20, 2.1.x→1.7.10, 2.2.x→1.7.20, 2.3.x→1.8.10, 3.0.x→2.0.20, 3.1.x→2.1.10, 3.2.x→2.1.21, 3.3.x→2.2.10, 3.4.x→2.3.0, 3.5.x→2.3.21 — confirmed live via each minor's own tagged `gradle.properties`/`libs.versions.toml`) — resolving a major to its absolute latest patch (this project's universal convention everywhere else) broke a real build: Ktor 3's latest patch (3.5.2, built with Kotlin 2.3) failed to compile under a Kotlin 2.1 target ("Module was compiled with an incompatible version of Kotlin"). Fixed with a Kotlin-line-aware resolver (`_resolve_ktor()`) that picks the highest Ktor minor whose own pinned Kotlin requirement doesn't exceed the target line, then resolves that minor's own latest patch — Kotlin binary metadata is forward-readable but not backward-readable, so this direction of the check is the one that matters. Maven anchor artifact itself also changed at the Kotlin-Multiplatform split (`ktor-server-core` pre-2.0, `ktor-server-core-jvm` from 2.0.0-beta-1 on) though the actual `build.gradle.kts` dependency line stays un-suffixed at every major (Gradle's own GMM-aware resolver picks the right platform variant automatically). |
| Spring Boot | 2, 3, 4 | Reuses Java's own tracked majors/artifact, narrowed to only the ones with real Kotlin support — Boot 1 is NOT tracked (a genuine impossibility, not a convenience exclusion: Kotlin support only entered the Spring stack at Framework 5.0, Jan 2017, three years after Boot 1). Boot 2's own per-version Kotlin floor isn't directly documented, so this project's established lifecycle-window rule (same one Java's own Micronaut/Helidon notes use) applies; Boot 3/4's floors ARE directly documented (3.0.0 pins `kotlinVersion=1.7.21` in its own `gradle.properties`; 4.0's own release blog states an explicit "Kotlin 2.2 baseline"). Deliberately built with Gradle's plain `application` plugin rather than the `org.springframework.boot`/`io.spring.dependency-management` Gradle plugins, to avoid adding a second, separately-versioned build-tool-plugin axis on top of Boot's own version — a direct `spring-boot-starter-web` dependency resolves its own transitive versions from its own POM regardless of whether a project also imports the parent BOM. **A real runtime bug found via `docker run`+`curl`, not the build**: Spring Boot's embedded Tomcat defaults to port 8080, not this project's uniform 8000 — unlike Ktor (where the port is an explicit `embeddedServer()` argument in the app code itself), Spring Boot needed an explicit `application.properties` with `server.port=8000`, the same mechanism Java's own `_PORT_CONFIG` already uses for its Spring Boot entry; missing this made the container start and answer correctly on 8080 while its published 8000 mapping got nothing at all. |
| http4k | 5, 6 | The second genuinely Kotlin-native framework — purely-functional, no annotations, no DI container (`HttpHandler = (Request) -> Response`, `routes(...)`/`bind` for dispatch, `asServer(Undertow(port))` to run). Narrowed to majors 5/6 (0-4 predate the version-resolution mechanism this project's own live checks depend on). **The densest real-world instance yet of the Ktor-style "latest patch needs a newer Kotlin than the target line" problem**: http4k releases essentially continuously (100+ real minors per major, an automated release-per-merge cadence) and its own required Kotlin compiler version drifts CONTINUOUSLY within a single major, not just at boundaries — confirmed via a real, dense sweep of `versions.properties`/`libs.versions.toml` across dozens of tags (e.g. within major 6 alone: 6.0-6.4 need Kotlin 2.0.21, 6.5-6.17 need 2.1.20, 6.18-6.24 need 2.2.20, 6.25-6.54 need 2.3.x, 6.55+ need 2.4.0). A full per-minor floor table the way Ktor's ~10 minors allowed isn't practical to hand-verify at this density, so `_resolve_http4k()` instead resolves to one of a small number of hand-picked, individually-verified EXACT checkpoint versions per major — a deliberate, documented deviation from this registry's "always resolve to latest patch" convention, not an oversight. |
| Micronaut | 1, 2, 3, 4, 5 | Reuses Java's own tracked majors/JDK facts wholesale. **Two real, compounding bugs found only via `docker run`, both invisible from a passing build**: (1) `io.micronaut:micronaut-inject-kotlin` as its own artifact only exists from Micronaut 4.0.0 onward (majors 1-3 have no such artifact at all); (2) that 4.x/5.x jar registers *only* a KSP `SymbolProcessorProvider` service, no `javax.annotation.processing.Processor` at all (confirmed via direct jar inspection) — so **kapt silently does nothing**: the build succeeds with zero errors, `kaptKotlin` runs, but zero routes ever get registered (both endpoints 404, confirmed live). Fixed with a genuine per-era mechanism split: majors 1-3 use kapt against `io.micronaut:micronaut-inject-java` instead (confirmed via jar inspection at both a major-1-era and major-3-era version: this one DOES implement the Processor SPI — the original pre-KSP mechanism, kapt generates Java stubs and this processes them like real Java sources); majors 4/5 use the real `id("com.google.devtools.ksp")` Gradle plugin, resolved to the exact KSP release matching the target Kotlin patch (versioned `{kotlin patch}-{ksp release}`, e.g. `2.0.21-1.0.28` — needs a raw, unfiltered Maven-metadata fetch since this hyphenated scheme doesn't match the project's shared stable-release regex). **A third bug, also real-build-only**: Micronaut's own `micronaut-parent`/`micronaut.platform:micronaut-parent` BOM coordinate has version numbers completely independent of `micronaut-core`'s (confirmed via "Could not find io.micronaut.platform:micronaut-parent:4.10.26", a version that plainly never existed under that groupId) — framework-version resolution must target the BOM coordinate directly, mirroring `lang_java.py`'s own precedent, never `micronaut-core`. Major 1's own Kotlin floor is narrowed from 1.2 to 1.6 after Kotlin 1.3 (this project's own JDK-8/Gradle-5.6.4 tier) failed kapt itself with an opaque `InvocationTargetException` — a genuine old-JDK-8-era kapt bug, confirmed by the identical template succeeding cleanly one tier up at Kotlin 1.6 (JDK 11), which real-generated 3 bean classes and answered both endpoints correctly. |
| Quarkus | 1, 2, 3 | Reuses Java's own tracked majors/JDK facts. Kotlin support (`quarkus-kotlin`) has existed since Quarkus's very first release, so unlike Spring Boot there's no "didn't exist yet" floor. Unlike Micronaut (deliberately skips the official Gradle plugin), Quarkus genuinely REQUIRES its own build-time bytecode-augmentation step to function at all, so the real `id("io.quarkus")` plugin is used. **A chain of four real, build/run-only bugs, none visible from a passing build in isolation**: (1) Quarkus's latest 3.x patch (3.38.1) needs Gradle ≥8.14 ("This version of Quarkus is tested with Gradle 8.14 or later", a real `NoClassDefFoundError` on an older Gradle) — but bumping the Gradle version alone doesn't fix everything, because (2) Kotlin 2.0.21's own Kotlin Gradle Plugin has an INDEPENDENT compatibility *ceiling* with newer Gradle: the identical compile failure (`IncrementalCompilationFeatures.<init>`, a binary API mismatch) reproduced under BOTH Gradle 8.14.2 and 9.6.0, ruling out "just pick a newer Gradle" — fixed by narrowing Quarkus's own Kotlin-compatibility to just 2.3/2.4 (the only tracked lines whose own default toolchain tier already sits at Gradle 9.6.0), a genuine technical impossibility for this framework specifically, not a convenience exclusion (every excluded Kotlin line still works fine with the other four frameworks). (3) The build-time `quarkus.package.jar.type=uber-jar` property has no Gradle-DSL equivalent of a Maven `<properties>` entry — it must be set in `application.properties`, the one file Quarkus's own augmentation step reads at build time too; without it Quarkus silently defaults to its fast-jar directory layout instead of the single jar this project's Dockerfile CMD expects, and the resulting filename bakes Gradle's own default "unspecified" project version in verbatim (`app-unspecified-runner.jar`) unless an explicit `version = "0.0.0"` is set. (4) The subtlest: `quarkus-bom` (imported as `enforcedPlatform`, a genuine requirement not a convenience choice) also manages `org.jetbrains.kotlin:kotlin-stdlib`'s own version, silently overriding the `kotlin("jvm")` plugin's own declared version for the STDLIB specifically — confirmed via a real build where `KotlinVersion.CURRENT` reported 2.4.0 despite an explicit "2.3.21" compiler declaration, which would have made two different tracked Kotlin lines silently indistinguishable at runtime for this one framework; fixed with an explicit `resolutionStrategy.force("org.jetbrains.kotlin:kotlin-stdlib:...")`, Gradle's own documented override mechanism for exactly this situation. |

A fifth, genuinely cross-cutting bug was found while testing Micronaut's oldest tier and affects **every** framework sharing the common `_gradle_build_kts()` template (Ktor/Spring Boot/http4k/Micronaut, including combos already shipped before this pass): Gradle's `application.mainClass` (a `Property<String>`) didn't exist before Gradle 6.4 (only the older `mainClassName` did), and the Kotlin Gradle Plugin's own `kotlin { jvmToolchain(...) }` DSL function didn't exist before KGP 1.7.0 — both confirmed via real "Unresolved reference" build failures under the Kotlin 1.3 tier (Gradle 5.6.4). Fixed with two independent era-boundary checks (`_OLD_MAINCLASS_LINES`, `_OLD_JVM_TARGET_LINES` — they don't share the same cutoff), falling back to `mainClassName = "..."` and the older, universally-supported `tasks.withType<KotlinCompile> { kotlinOptions.jvmTarget = "..." }` for Kotlin lines old enough to need it.

| Crypto library | Version buckets | Notes |
|-----------------|------------------|-------|
| JCA | built-in | Identical to Java's own entry — same JVM bytecode, zero wrapper needed. |
| BouncyCastle | 1.70 through 1.85 (16 buckets) | Same coordinate/dates/PQC-support facts as Java's own entry, reused wholesale (`org.bouncycastle:bcprov-jdk18on`, plus the `1.70`-only legacy `bcprov-jdk15on` switch) — plain-Java bytecode, no Kotlin sensitivity. |
| Tink | 1.7 through 1.22 (16 buckets) | Same coordinate/PQC facts as Java's entry, narrowed to 1.7 onward for this first pass (Java's own pre-1.7 history not mechanically duplicated) — same `AeadConfig.register()`/`registerStandardKeyTypes()` version split. |
| Conscrypt | 2.4, 2.5 | Same coordinate/arm64 platform limitation as Java's entry, narrowed to the two most recent stable minors for this first pass. |
| KotlinCrypto | 0.2 through 0.8 (7 buckets) | The one genuinely Kotlin-native (not JVM-reused-from-Java) library tracked — github.com/KotlinCrypto, anchored on `org.kotlincrypto.hash:sha2-jvm`. **Important negative finding, confirmed via its own module list**: classical-crypto only (hash/HMAC/secure-random/bit-ops) — no KEM, no signatures, no PQC support at all, and not a symmetric-cipher/TLS library either. Exists in this registry specifically to surface that contrast against liboqs-kotlin below. |
| liboqs-kotlin | 0.3.0 | liboqs-java (the official OQS-org JNI wrapper), consumed unchanged from Kotlin (plain `org.openquantumsafe.KeyEncapsulation`/`.Signature` API). **Confirmed NOT published to Maven Central** (a real 404) — built from source in a dedicated `liboqs-builder` stage (git-clone liboqs C library at a pinned tag → cmake/ninja static build+install → git-clone liboqs-java → `mvn package`), the same multi-stage shape as PHP's own `php-liboqs` handling, reused as precedent rather than invented fresh. Registry `module` is the literal marker string `"source-build"` (parallel to Node's own `"built-in"` marker) so `check_updates.py`'s generic Maven enumerator correctly skips it instead of crashing on a bad `split(":", 1)`. **Three real, build/run-only bugs found and fixed, none visible from a passing build alone**: (1) the git tag for liboqs-java carries no `v` prefix (`0.3.0`, not `v0.3.0` — the natural guess) confirmed only by a real "Remote branch not found" clone failure; (2) the touch code's `cleanup()` method doesn't exist on `KeyEncapsulation` — the real teardown method, confirmed against liboqs-java's own `KEMExample.java`, is `dispose_KEM()`; (3) the most subtle one — liboqs-java's own `mvn package` (native-maven-plugin, Linux profile) produces a `liboqs-jni.so` that references OpenSSL's `CRYPTO_malloc` as an undefined dynamic symbol but never records `libcrypto.so.3` in its own `NEEDED` list, causing a real `symbol lookup error` at container startup (confirmed via `readelf`/`nm` inspection extracting the `.so` straight out of the built jar) — fixed with `patchelf --add-needed libcrypto.so.3` on the `.so` before it gets re-packed into the jar, relying on OpenSSL 3.x's own stable-ABI guarantee to make this safe even though the builder (Ubuntu 24.04, OpenSSL 3.0.13 at research time) and runtime (`eclipse-temurin:{jdk}-jre`, Ubuntu 26.04, OpenSSL 3.5.5) ship different libcrypto minors. |

**Build-verification status.** Real `docker build`+`run`+curl (both endpoints) confirmed for: Ktor 2 + Kotlin 2.0 + JCA (baseline, no fixes needed), Spring Boot 4 + Kotlin 2.2 + BouncyCastle, Ktor 3 + Kotlin 2.1 + liboqs-kotlin (a live ML-KEM-768 keypair generation), http4k 6 + Kotlin 2.2 + JCA (baseline, no fixes needed), Micronaut 4 + Kotlin 2.0 + BouncyCastle (KSP path), Micronaut 1 + Kotlin 1.6 + JCA (kapt path, after narrowing off the broken 1.2-1.5/JDK-8 window), and Quarkus 3 + Kotlin 2.3 + Tink (after all four Quarkus-specific fixes plus the cross-cutting old-Gradle-DSL fix, with the reported `language.version` re-verified to read "2.3.21" and not the silently-forced "2.4.0" from before the stdlib fix). A real local generation pass (3,655 image contexts, 0 exceptions) was also run before any Docker build was attempted. Every other combination shares this same `build.gradle.kts`/Dockerfile-shape machinery per framework/library, so an untested combination's risk is concentrated in whichever specific branch it exercises (a particular Ktor/http4k minor's own Kotlin-compiler pin, a particular JDK/Gradle toolchain tier, Micronaut's kapt-vs-KSP split), not the shared plumbing.

**Post-v1.10 hardening (v1.10.1 through v1.10.9), all found via the user's own bulk build/test runs on real hardware, not anticipated in the original pass.** A durable pattern emerged across most of these: a framework/library resolves cleanly to *some* version that satisfies its own registry-tracked Kotlin-compiler-compatibility floor, but that specific version *also* carries an independent, unrelated floor (a JVM bytecode target, a Gradle-plugin requirement, a transitive dependency's own pin) that the target Kotlin line's toolchain tier doesn't satisfy — Kotlin-compiler compatibility and JVM/Gradle-toolchain compatibility are separate axes, and checking one says nothing about the other:
- **Ktor major 1** turned out to have the identical per-minor (later found to be per-*patch*) Kotlin-pin drift already fixed for majors 2/3 — the static per-minor floor table proved insufficient even for the minors it was built to cover (Ktor 1.6.x alone drifts across 8 patches, one under a third config key, `kotlin-version` with a hyphen). `_resolve_ktor()` was rewritten from a static table to a live, cached, per-patch walk — newest-first until a patch's own real pin is found to satisfy the target line — a fundamentally more robust shape than any hand-sampled table for a framework this volatile.
- **Tink** needed the `google()` Maven repo (a real, non-optional `androidx.annotation` dependency only published there) and its 1.19+ buckets needed narrowing to Kotlin 1.5+ (the same "dropped Java 8" fact already known from Java's own registry, just forgotten when this entry was written fresh). **KotlinCrypto's own declared build-tooling Kotlin version turned out to be a red herring for consumer compatibility** — real floors came from reading a genuine batch-test pass/fail matrix directly, not from the library's own `libs.versions.toml`.
- **Micronaut 5** forces a JDK 25 toolchain (needing a per-major override, same shape as Quarkus's own Gradle-floor override); Kotlin 2.2 specifically has a genuine, unresolvable conflict with it (2.2's compiler caps its own bytecode target at 24, predating JDK 25's GA, while Micronaut 5.1.10's Gradle Module Metadata hard-requires ≥25) — narrowed to 2.3/2.4 only after confirming an explicit `jvmTarget=24` override just moved the conflict rather than fixing it. KSP separately changed its own versioning scheme exactly at Kotlin 2.3.0 (hyphenated `{kotlin}-{ksp}` → direct hyphen-less match), silently producing a broken `version "None"` literal until both the parser and a write-time availability guard were fixed.
- **Quarkus majors 1/2** turned out to be genuinely incompatible with the Gradle 9.6.0 tier this registry pairs with Kotlin 2.3/2.4 — their own Gradle plugins are EOL and were never updated for it (major 1 calls the since-removed `JavaPluginConvention`; major 2 hits a real, documented quarkusio/quarkus Gradle-plugin bug around newer Gradle's auto-registered test-suite configs). Fixed with a per-major toolchain override (Gradle 7.6.4/JDK 11) rather than narrowing the registry, since both majors build and run cleanly once paired with an old-enough Gradle. Separately, **Tink 1.16+ embeds a protobuf runtime-version check** that Quarkus majors 1/2's own `quarkus-bom` (imported `enforcedPlatform`) silently downgraded by pinning `protobuf-java` to a pre-runtime-version-check release — fixed with a second `resolutionStrategy.force()` (Tink's own live-fetched POM property, not hardcoded) alongside the existing kotlin-stdlib one.
- **Spring Boot**: `jackson-module-kotlin` (a direct dependency, not a registry axis) was always resolved to Maven's absolute latest, on the wrong assumption that any recent release is compatible with whatever Boot pulls in — its own `<version.kotlin>` POM property drifts over time exactly like Ktor's per-patch pin, so `_resolve_jackson_kotlin()` (the same live per-patch-walk shape as `_resolve_ktor()`) replaced it. That fix alone was still insufficient for Boot 3: `spring-boot-starter-web` transitively imports `com.fasterxml.jackson:jackson-bom` as a `platform()` at whatever Jackson version that Boot patch resolved to, which *also* manages `jackson-module-kotlin`'s version and — since a `platform()` recommendation participates in normal conflict resolution — silently outvoted the deliberately-older resolved version, reproducing the same "incompatible version of Kotlin" failure a second time; fixed with `resolutionStrategy.force()`. Separately, Boot 3 has required JDK ≥17 since its first release, conflicting with Kotlin 1.7's own JDK-11 toolchain tier — a per-(Spring Boot 3, Kotlin 1.7) override bumps just that one pairing to JDK 17.
- **http4k 6** set its minimum JVM bytecode target to 21 for the whole major from its first release (its own release blog: "From v6, http4k CE will set its minimum supported target JVM version to 21") — independent of which Kotlin-compatible checkpoint gets resolved, confirmed both via a real `UnsupportedClassVersionError` and by directly inspecting a checkpoint's own compiled `.class` bytes (`cafe babe 0000 0041`, major version 65 = JDK 21). Conflicted only with Kotlin 2.0 (this registry's JDK-17 tier); every other http4k-6-compatible line already defaults to JDK 21+.

Three of these nine post-v1.10 fixes (Quarkus's Gradle floor, Micronaut 5's JDK floor, http4k 6's JDK floor) are the same underlying bug class found independently three times — worth checking explicitly for both a Kotlin-compiler floor *and* a JVM/Gradle-toolchain floor on any future framework addition, not just the former.
...
### When a language re-resolves dependencies at build time (no committed lockfile per image) — lessons from Rust
Rust has no committed `Cargo.lock` per generated image, so every `docker build` re-resolves every transitive dependency to its newest semver-compatible release, and the crates.io ecosystem keeps drifting forward underneath old, previously-passing targets. **This exact shape recurs for any language whose package manager separates "what I asked for" from "what got installed" without a project-level lockfile being generated and shipped per image** — most immediately relevant to a future Ruby addition (Bundler/`Gemfile.lock`), but worth checking for any new language before assuming a passing build today will still pass tomorrow:
- **A dependency's own declared compatibility metadata (MSRV/`rust_version`, npm `engines`, PyPI `python_requires`, a gem's own declared Ruby version, etc.) is a maintainer courtesy, not enforced or verified — it can be missing, wrong, or simply understate the real floor.** Rust hit all three: crates with NO declared floor at all whose real one could only be found by actually compiling (`tinyvec`, `subtle`); crates whose declared floor was flatly incorrect once build-tested (`spin`, `rsa` 0.8's real floor 3 versions above its own crates.io-declared one); and a floor invisible to metadata entirely — cargo's own package-index SCHEMA version (namespaced/weak-dependency-feature syntax), which isn't exposed by the crate's REST API at all and needed its own separate fetch against the raw sparse index. **The only fully reliable way to establish a real compatibility floor is a real trial build against the candidate runtime version — metadata is a starting hint, never proof.**
- **If a "repair"/pin step runs in one build stage and the real build happens in a later one (e.g. a throwaway resolver stage vs. a multi-stage Dockerfile's builder stage), verify EXACTLY which artifact crosses that boundary — a lockfile-shaped fix that only lives in a file the next stage never sees silently does nothing.** Rust's fix could only pin the manifest itself (`Cargo.toml`), never a lockfile, because the target rustc's own (possibly very old) toolchain has to do its own native, deterministic resolution from an exact-pinned manifest — a modern-tool-written lockfile can be in a newer on-disk format the old toolchain can't even parse, regardless of its version content.
- **Any mechanism that queries a live package registry API once per build (to check a real version floor, fetch metadata, etc.) is a real, recurring rate-limit risk once builds run in any volume or parallelism — and a naive retry-then-give-up implementation can silently corrupt correctness, not just fail slowly.** Rust's own repair script originally treated an exhausted-retry fetch as "this package has no floor constraint" instead of an unknown — quietly producing false floors across an entire batch with no visible error. Retry with exponential backoff, and on total exhaustion either fall back to a clearly-marked stale cache or **fail the whole build loudly** — never silently substitute "unknown" for "safe." Separately, add a persistent, TTL'd, on-disk cache for these lookups (living inside whatever build cache is already shared across builds) — a large batch where every single build independently re-fetches the same few hundred packages from scratch is what actually sustains rate-limiting, not any one slow query.
- **A BuildKit cache-mount's `sharing=locked` vs `sharing=shared` setting is a real trade-off, not a strict upgrade, for any language's own package-manager cache directory.** `locked` serializes every concurrent build through one global lock per cache ID — safe, but can starve an unlucky build for a very long time (1h40m+ observed) under real parallelism, since it can keep losing the lock race to later-arriving builds indefinitely. `shared` allows true concurrency (relying on the tool's own internal advisory locking — e.g. cargo's) but trades that for rare first-extraction races (two builds racing to write the identical fresh cache entry for the first time) — confirm any such race is transient via a direct retry before treating it as a real bug. Also: a `--no-cache` build gets an ISOLATED view of a `sharing=shared` mount (its own writes don't merge back for a LATER `--no-cache` build to see) — only a later non-`--no-cache` build correctly sees them, so don't use chained `--no-cache` runs to test whether a cache-persistence fix actually works.
- **A transitive "ecosystem hub" dependency's real compatibility wall can be library-dependent, not just framework-dependent, even within the same framework's own version bucket.** Don't assume one bisected threshold applies uniformly across an entire framework major — different libraries paired with the same framework can reach the same underlying hub crate through different paths in their own dependency graph, at different thresholds. Bisect per combination when it matters, not just once per framework.
- **When debugging by reproducing a build step in isolation, use the exact same base image the real pipeline stage uses for that step — not the target/runtime language version's own image.** An early resolver/repair stage typically pins itself to a fixed, modern base specifically so it has capabilities (e.g. a newer stdlib) the eventual old target version's own image may lack; instrumenting with the wrong image can produce a misleading crash that has nothing to do with the real bug.
...
- **New-versions scanner (`scripts/check_updates.py`) — MANDATORY, always add this in the same pass as the registry/generator, not as an optional follow-up.** Needs: (1) a `"{lang}": "registry {lang}.json"` entry in `_REGISTRY_FILES`; (2) a new enumerator in `_ENUMERATORS` — if the new language's registry.json `frameworks`/`cryptography_libs` entries carry the package identifier directly in their own `"module"` field (true for every language so far except Python), this is a **one-line lambda** reusing the existing `_enumerate_registry_module(d, "<fetch_kind>")` helper unchanged, no new function needed (see Rust's `"rust": lambda d: _enumerate_registry_module(d, "crates")` for the reference case); (3) a new `fetch_kind` branch in both `_fetch()` **and** `_fetch_date()`, each just delegating to a `_fetch_releases`-equivalent/`_release_date` pair the new language's own `lang_X.py` should already have from implementing version resolution in the first place (Rust already had `_fetch_crates_versions`/`_release_date` for exactly this reason — check before writing a new fetch function, don't duplicate). Verify with `python scripts/check_updates.py --lang {lang} --dry-run` before considering it done — it should run against the real upstream registry and report genuine untracked majors with no crash. Skipping this doesn't error anywhere else, it just means the new language's frameworks/libraries silently never surface in the "🔔 New versions" badge at all — which is exactly why it's easy to forget and exactly why it must be done proactively, not just when a gap is later noticed. Status: done for Python/Node/Java/.NET/PHP/Rust.
- **Client-side fingerprinting (`CONTEXT_CLIENTS.md`)**: currently Python-only (`scripts/lang_python_client.py` + `scripts/generate_client_images.py`). A new server-side language does NOT automatically get client images — that needs its own `lang_X_client.py` (one-shot outbound-call generator per crypto library, following the Python client templates' pattern) wired into `generate_client_images.py`, plus registry entries under that language's `http_clients` section. Deliberately deferred for Rust for now (2026-08-01) — revisit when client-side work resumes generally, not specific to Rust.
