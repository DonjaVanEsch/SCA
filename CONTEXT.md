...
**Build-verification status.** Real `docker build`+`run`+curl (both endpoints) confirmed for: Ktor 2 + Kotlin 2.0 + JCA (baseline, no fixes needed), Spring Boot 4 + Kotlin 2.2 + BouncyCastle (caught the port-config bug above), and Ktor 3 + Kotlin 2.1 + liboqs-kotlin (caught all three liboqs-kotlin fixes above, including a live ML-KEM-768 keypair generation). A real local generation pass (1,161 image contexts, 0 exceptions) was also run before any Docker build was attempted. Every other combination shares this same `build.gradle.kts`/Dockerfile-shape machinery per framework/library, so an untested combination's risk is concentrated in whichever specific branch it exercises (a particular Ktor minor's own Kotlin-compiler pin, a particular JDK/Gradle toolchain tier), not the shared plumbing.
