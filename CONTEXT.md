...
Rust was the next priority language and is now fully implemented — see "Rust (7th language ...)" under "What is already covered" above, plus the general lessons its `msrv_repair.py` and BuildKit-cache work surfaced in "Conventions to follow when adding a new language" below (most relevant to Ruby, whose Bundler/Gemfile.lock situation is architecturally similar: no lockfile committed per generated image means the same "unpinned transitive dependency drifts over time" risk applies there too).
